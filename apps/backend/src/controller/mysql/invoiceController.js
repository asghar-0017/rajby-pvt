...
export const bulkCreateInvoices = async (req, res) => {
  ...
  try {
    ...
    const existingBuyers = uniqueBuyerNTNs.length > 0 ? 
      await Buyer.findAll({
        where: { buyerNTNCNIC: uniqueBuyerNTNs },
        attributes: ['buyerNTNCNIC', 'buyerBusinessName', 'buyerProvince', 'buyerAddress', 'buyerRegistrationType']
      }) : [];
    ...
    for (let i = 0; i < invoices.length; i++) {
      ...
      try {
        // Quick validation
        if (!invoiceData.invoiceType?.trim() || !invoiceData.invoiceDate?.trim() || 
            !invoiceData.buyerBusinessName?.trim() || !invoiceData.buyerProvince?.trim()) {
          errors.push({
            index: i,
            row: i + 1,
        ...
        if (!["Sale Invoice", "Debit Note"].includes(invoiceData.invoiceType.trim())) {
          errors.push({
            index: i,
            row: i + 1,
        ...
        const validProvinces = [
          "Balochistan", "Azad Jammu and Kashmir", "Capital Territory", "Punjab",
          "Khyber Pakhtunkhwa", "Gilgit Baltistan", "Sindh",
          "BALOCHISTAN", "AZAD JAMMU AND KASHMIR", "CAPITAL TERRITORY", "PUNJAB",
          "KHYBER PAKHTUNKHWA", "GILGIT BALTISTAN", "SINDH"
        ];
        ...
        if (invoiceData.buyerNTNCNIC?.trim()) {
          ...
          if (existingBuyer) {
            ...
            newBuyers.push({
              ...
              buyerRegistrationType: invoiceData.buyerRegistrationType || "Unregistered"
            });
          }
        }
        ...
        if (systemInvoiceId.length > 20) {
          ...
          if (fallbackId.length > 20) {
            throw new Error(`Fallback system_invoice_id too long: "${fallbackId}" (${fallbackId.length} chars, max 20)`);
          }
        ...
        const itemFields = [
          "item_hsCode", "item_productDescription", "item_rate", "item_uoM", "item_quantity",
          "item_unitPrice", "item_totalValues", "item_valueSalesExcludingST",
          "item_fixedNotifiedValueOrRetailPrice", "item_salesTaxApplicable",
          "item_salesTaxWithheldAtSource", "item_extraTax", "item_furtherTax",
          "item_sroScheduleNo", "item_fedPayable", "item_discount", "item_saleType",
          "item_sroItemSerialNo"
        ];

        // Check if this invoice has item data
        const hasItemData = itemFields.some(field => 
          invoiceData[field] !== undefined && invoiceData[field] !== null && invoiceData[field] !== ""
        );
    ...
    console.log(`📈 Processed ${invoices.length}/${invoices.length} invoices...`);
    ...
    for (let i = 0; i < invoiceBatches.length; i++) {
      ...
      maxSystemIdLength = Math.max(maxSystemIdLength, invoice.system_invoice_id.length);
      maxInvoiceNumberLength = Math.max(maxInvoiceNumberLength, invoice.invoice_number.length);
      
      // Check system_invoice_id length (max 20 chars)
      if (invoice.system_invoice_id.length > 20) {
        throw new Error(`system_invoice_id too long at row ${i + 1}: "${invoice.system_invoice_id}" (${invoice.system_invoice_id.length} chars, max 20)`);
      }
      
      // Check invoice_number length (max 100 chars)
      if (invoice.invoice_number.length > 100) {
        throw new Error(`invoice_number too long at row ${i + 1}: "${invoice.invoice_number}" (${invoice.invoice_number.length} chars, max 100)`);
      }
      
      // Check other string fields
      if (invoice.buyerBusinessName && invoice.buyerBusinessName.length > 255) {
        throw new Error(`buyerBusinessName too long at row ${i + 1}: ${invoice.buyerBusinessName.length} chars, max 255`);
      }
      
      if (invoice.buyerProvince && invoice.buyerProvince.length > 100) {
        throw new Error(`buyerProvince too long at row ${i + 1}: ${invoice.buyerProvince.length} chars, max 100`);
      }
    }
    
    console.log(`✅ Validation passed - Max lengths: system_invoice_id=${maxSystemIdLength}, invoice_number=${maxInvoiceNumberLength}`);
    ...
    const result = await sequelize.transaction(async (t) => {
      ...
      if (invoiceBatches.length > 0) {
        ...
        if (invoiceBatches.length > 1) {
          console.log('📝 Additional samples:', {
            ...
            row_100: invoiceBatches.length > 100 ? {
        ...
        const longIds = invoiceBatches.filter(inv => inv.system_invoice_id.length > 20);
        if (longIds.length > 0) {
          console.error('❌ Found IDs that are too long:', longIds.slice(0, 3).map(inv => ({
            system_invoice_id: inv.system_invoice_id,
            length: inv.system_invoice_id.length
          })));
          throw new Error(`Found ${longIds.length} system_invoice_ids that exceed 20 character limit`);
        }
      }
      ...
      try {
        ...
        if (bulkError.errors && bulkError.errors.length > 0) {
          ...
          throw new Error(`Bulk create failed: ${firstError.message} (Field: ${firstError.path}, Value: ${firstError.value})`);
        }
        ...
        if (bulkError.parent && bulkError.parent.code) {
          ...
          if (bulkError.parent.code === 'ER_DATA_TOO_LONG') {
            throw new Error(`Data too long for database column. Check field lengths in your data.`);
          }
        }
      ...
      if (invoiceItemBatches.length > 0) {
        console.log(`🔄 Creating ${invoiceItemBatches.length} invoice items...`);
        
        const updatedItemBatches = invoiceItemBatches.map((item, index) => {
          // Find corresponding invoice
          const invoiceIndex = Math.floor(index / (invoiceItemBatches.length / invoiceBatches.length));
        ...
        if (updatedItemBatches.length > 0) {
          try {
            await InvoiceItem.bulkCreate(updatedItemBatches, { 
              transaction: t,
              fields: Object.keys(updatedItemBatches[0]).filter(key => key !== 'invoice_id_placeholder'),
              validate: true
            });
          } catch (itemError) {
    ...
    console.log(`📊 Summary: ${result.length} invoices created, ${errors.length} errors, ${warnings.length} warnings`);
    ...
    const totalTime = Number(process.hrtime.bigint() - startTime) / 1000000;
    console.error(`❌ Bulk upload failed after ${totalTime.toFixed(2)}ms:`, error);
