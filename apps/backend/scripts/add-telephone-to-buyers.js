...
/**
 * Migration script to add buyerTelephone column to buyers table
 * This script will add the telephone column to all tenant databases
 */
async function addTelephoneToBuyers() {
  let connection;
  ...
  try {
    ...
    // Get all databases (tenant databases)
    const [databases] = await connection.execute(`
      SELECT SCHEMA_NAME 
      FROM INFORMATION_SCHEMA.SCHEMATA 
      WHERE SCHEMA_NAME NOT IN ('information_schema', 'mysql', 'performance_schema', 'sys')
      AND SCHEMA_NAME LIKE '%tenant%' OR SCHEMA_NAME LIKE '%innovative%'
    `);

    const tenantDatabases = databases.map((db) => db.SCHEMA_NAME);
    console.log(
...
// Run the migration
addTelephoneToBuyers().catch(console.error);
