...
async function testBackupSystem() {
  ...
  try {
    ...
    const [backupTables] = await connection.execute("SHOW TABLES LIKE 'invoice_backups'");
    const [summaryTables] = await connection.execute("SHOW TABLES LIKE 'invoice_backup_summary'");
    ...
    const [backupColumns] = await connection.execute("DESCRIBE invoice_backups");
    const [summaryColumns] = await connection.execute("DESCRIBE invoice_backup_summary");
    
    console.log(`   📊 invoice_backups has ${backupColumns.length} columns`);
    console.log(`   📊 invoice_backup_summary has ${summaryColumns.length} columns`);
    
    // Check for key columns
    const backupColumnNames = backupColumns.map(col => col.Field);
    const requiredBackupColumns = ['id', 'original_invoice_id', 'backup_type', 'invoice_data', 'created_at'];
    ...
    for (const col of requiredBackupColumns) {
      if (backupColumnNames.includes(col)) {
        ...
        console.log(`   ❌ Required column '${col}' missing in invoice_backups`);
      }
    }
    ...
    const [backupIndexes] = await connection.execute("SHOW INDEX FROM invoice_backups");
    const [summaryIndexes] = await connection.execute("SHOW INDEX FROM invoice_backup_summary");
    
    console.log(`   📊 invoice_backups has ${backupIndexes.length} indexes`);
    console.log(`   📊 invoice_backup_summary has ${summaryIndexes.length} indexes`);
    ...
    if (enumValues.length > 0) {
      ...
      const expectedTypes = ['DRAFT', 'SAVED', 'EDIT', 'POST', 'FBR_REQUEST', 'FBR_RESPONSE'];
    ...
    try {
      ...
      console.log(`   ✅ Test backup inserted successfully (ID: ${result.insertId})`);
      
      // Clean up test data
      await connection.execute('DELETE FROM invoice_backups WHERE id = ?', [result.insertId]);
