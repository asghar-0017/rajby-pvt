  ...
  try {
    ...
    if (log.new_values) {
      ...
      if (newValues.invoice_items) {
        console.log(
          `   ✅ invoice_items: ${newValues.invoice_items.length} items`
        );
    ...
    if (log.old_values) {
      ...
      if (oldValues.invoice_items) {
        console.log(
          `   ✅ invoice_items: ${oldValues.invoice_items.length} items`
        );
      } else {
        console.log(`   ❌ invoice_items: Missing`);
      }
    ...
    console.log(
      "   4. The invoice items should now be displayed in structured tables"
    );
  } catch (error) {
