...
async function analyzeAuditPermissionIndexes() {
  let connection;
  try {
    connection = await createConnection(dbConfig);
    console.log('✅ Connected to database successfully');

    console.log('\n🔍 Analyzing audit_permissions table indexes...');
    
    const [indexes] = await connection.execute('SHOW INDEX FROM audit_permissions');
    ...
    Object.entries(columnUsage).forEach(([column, indexNames]) => {
      if (indexNames.length > 1) {
        console.log(`  - ${column}: ${indexNames.join(', ')} (${indexNames.length} indexes)`);
      }
    });
    
    // Check table structure
    const [columns] = await connection.execute('SHOW COLUMNS FROM audit_permissions');
    ...
    if (totalIndexes > totalColumns * 2) {
      console.log(`  ⚠️  High index-to-column ratio (${(totalIndexes/totalColumns).toFixed(1)}x)`);
    }
