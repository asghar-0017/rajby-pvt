  ...
  try {
    ...
    for (const table of tables) {
      ...
      if (indexes.length > 50) {
        console.log(
          `⚠️  Table ${tableName} has ${indexes.length} indexes (high count)`
        );
        ...
        if (duplicateIndexes.length > 0) {
          console.log(
            `   🔍 Found ${duplicateIndexes.length} potentially duplicate indexes`
          );
        ...
        if (redundantIndexes.length > 0) {
          console.log(
            `   🔍 Found ${redundantIndexes.length} potentially redundant indexes`
          );
        ...
        console.log(
          `   ✅ Table ${tableName} has ${indexes.length} indexes (acceptable)`
        );
      }
    }
      ...
      try {
        ...
        if (indexes.length > 30) {
          ...
          if (indexesToRemove.length > 0) {
            console.log(
              `   💡 Consider removing these indexes: ${indexesToRemove.slice(0, 5).join(", ")}`
            );
          }
        }
      } catch (error) {
    ...
    console.log(
      "   4. Consider using composite indexes instead of multiple single-column indexes"
    );
    console.log("");
    console.log(
      "⚠️  Note: This script only analyzes indexes. Manual cleanup may be required."
    );
  } catch (error) {
