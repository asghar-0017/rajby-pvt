    ...
    for (const tenant of tenants) {
      ...
      try {
        ...
        if (backupRecord.length > 0) {
          ...
          console.log(
            `      - Original Invoice ID: ${backup.original_invoice_id}`
          );
          console.log(`      - Backup Type: ${backup.backup_type}`);
          console.log(
            `      - Invoice Items Data: ${backup.invoice_items_data}`
          );
          ...
          console.log(
            `   📊 Original invoice has ${invoiceItems.length} items:`
          );
          invoiceItems.forEach((item, index) => {
            console.log(
              `      ${index + 1}. ${item.name || "Unnamed"} - Qty: ${item.quantity}, Price: ${item.unit_price}`
            );
          });
