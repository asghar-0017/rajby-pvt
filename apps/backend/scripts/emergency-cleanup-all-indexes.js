...
async function emergencyCleanupAllIndexes() {
  ...
  try {
      ...
      for (const [column, indexNames] of Object.entries(columnIndexes)) {
        if (indexNames.length > 1) {
          ...
          for (const indexName of indexesToRemove) {
            try {
              console.log(
                `   🗑️  Removing index: ${indexName} on column ${column}`
              );
              ...
              console.log(
                `   ⚠️  Could not remove index ${indexName}: ${error.message}`
              );
            }
          }
        }
    ...
    allTables.forEach((table) => {
      const status =
        table.index_count <= 10 ? "✅" : table.index_count <= 20 ? "⚠️" : "❌";
      console.log(
        `   ${status} ${table.table_name}: ${table.index_count} indexes`
      );
    });
