  ...
  try {
    ...
    for (const assignment of userRoleAssignments) {
      try {
        ...
        if (!role) {
          console.log(
            `Role '${assignment.roleName}' not found, skipping ${assignment.email}`
          );
          continue;
        }
        ...
        if (result.affectedRows > 0) {
          console.log(
            `✓ Assigned role '${assignment.roleName}' to ${assignment.email}`
          );
        } else {
        ...
        console.error(
          `Error assigning role to ${assignment.email}:`,
          error.message
        );
      }
    }
