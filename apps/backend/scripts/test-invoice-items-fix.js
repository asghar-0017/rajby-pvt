...
async function testInvoiceItemsFix() {
  ...
  try {
    ...
    const [tenants] = await connection.execute(
      "SELECT id, database_name, seller_business_name FROM tenants WHERE is_active = 1"
    );
    ...
    for (const tenant of tenants) {
      ...
      try {
        ...
        console.log(
          `   📊 Found ${invoicesWithItems.length} invoices with items:`
        );
        invoicesWithItems.forEach((invoice) => {
          console.log(
            `      - Invoice ${invoice.id}: ${invoice.invoice_number} (${invoice.status}) - ${invoice.item_count} items`
          );
        });
        ...
        recentBackups.forEach((backup) => {
          console.log(
            `      - Backup ${backup.id}: Invoice ${backup.original_invoice_id} (${backup.backup_type}) - ${backup.items_count} items`
          );
          if (backup.items_count === 0) {
            console.log(
              `        ⚠️  Empty invoice items: ${backup.invoice_items_data}`
            );
          }
        });
        ...
        console.error(
          `   ❌ Error checking tenant ${tenant.database_name}:`,
          error.message
        );
      }
    }
