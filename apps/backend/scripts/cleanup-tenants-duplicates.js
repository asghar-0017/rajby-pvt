...
async function cleanupTenantsDuplicates() {
  ...
  try {
    ...
    const tenantIdDuplicates = indexes
      .filter(
        (idx) => idx.Column_name === "tenant_id" && idx.Key_name !== "tenant_id"
      )
    ...
    const sellerNtnCnicDuplicates = indexes
      .filter(
        (idx) =>
          idx.Column_name === "seller_ntn_cnic" &&
          idx.Key_name !== "seller_ntn_cnic"
      )
      .map((idx) => idx.Key_name);
    ...
    console.log(
      `🔍 Found ${allDuplicates.length} duplicate indexes to remove:`
    );
    console.log(`  - tenant_id duplicates: ${tenantIdDuplicates.length}`);
    console.log(
      `  - seller_ntn_cnic duplicates: ${sellerNtnCnicDuplicates.length}`
    );
