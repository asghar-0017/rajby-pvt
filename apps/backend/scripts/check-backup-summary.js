...
async function checkBackupSummary() {
  ...
  const [summaries] = await connection.execute(
    "SELECT * FROM invoice_backup_summary ORDER BY id DESC LIMIT 5"
  );
  summaries.forEach((summary) => {
    console.log(
      `ID: ${summary.id}, Invoice ID: ${summary.original_invoice_id}, Invoice Number: ${summary.invoice_number}, Tenant: ${summary.tenant_name}`
    );
    console.log(
      `  - Created By: ${summary.created_by_name} (${summary.created_by_email})`
    );
    console.log(
      `  - Last Modified By: ${summary.last_modified_by_name} (${summary.last_modified_by_email})`
    );
