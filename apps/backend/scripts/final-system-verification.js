...
async function finalSystemVerification() {
  ...
  try {
    ...
    if (problematicTables.length === 0) {
      ...
      console.log(
        `❌ ${problematicTables.length} tables still have too many indexes`
      );
    }
    ...
    for (const tenant of tenants) {
      try {
        ...
        if (backupTables.length > 0 && summaryTables.length > 0) {
          console.log(
            `   ✅ ${tenant.seller_business_name}: Backup tables exist`
          );
          ...
          if (backupCount > 0) {
            console.log(
              `      📊 Recent backups: ${backupCount} (${withItems} with items)`
            );
          ...
          console.log(
            `   ❌ ${tenant.seller_business_name}: Backup tables missing`
          );
        }
        ...
        console.log(
          `   ❌ ${tenant.seller_business_name}: Error - ${error.message}`
        );
      }
    }
