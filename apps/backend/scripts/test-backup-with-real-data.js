...
async function testBackupWithRealData() {
  ...
  try {
    ...
    const user = users.length > 0 ? users[0] : { id: 5, email: 'system@fbr.com', first_name: 'System', last_name: 'User' };
    console.log(`👤 Using user: ${user.email} (${user.first_name} ${user.last_name})`);
    ...
    console.log(`✅ Real data backup created successfully (ID: ${backupResult.insertId})`);
    ...
    console.log(`✅ Real data backup summary created successfully (ID: ${summaryResult.insertId})`);
    ...
    if (storedSummary.length > 0) {
      ...
      console.log(`   - System Invoice ID: ${summary.system_invoice_id || 'NULL'}`);
      console.log(`   - FBR Invoice Number: ${summary.fbr_invoice_number || 'NULL'}`);
      console.log(`   - Tenant ID: ${summary.tenant_id || 'NULL'}`);
      console.log(`   - Tenant Name: ${summary.tenant_name || 'NULL'}`);
      console.log(`   - Created By: ${summary.created_by_name || 'NULL'} (${summary.created_by_email || 'NULL'})`);
      console.log(`   - Last Modified By: ${summary.last_modified_by_name || 'NULL'} (${summary.last_modified_by_email || 'NULL'})`);
