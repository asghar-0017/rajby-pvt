...
async function setupAuditSystem() {
  ...
  try {
    ...
    for (let i = 0; i < sqlFiles.length; i++) {
      ...
      if (fs.existsSync(sqlFilePath)) {
        ...
        try {
          ...
          console.log(
            `✅ File ${i + 1}/${sqlFiles.length} (${sqlFile}) executed successfully`
          );
        } catch (error) {
          if (
            ...
            console.log(
              `⚠️  File ${i + 1}/${sqlFiles.length} (${sqlFile}) skipped (already exists)`
            );
          } else {
            console.error(
              `❌ Error executing file ${i + 1}/${sqlFiles.length} (${sqlFile}):`,
              error.message
            );
            throw error;
          }
    ...
    console.log(
      `✅ ${rolePermissions.length} audit permissions assigned to roles:`
    );
    ...
    console.log(
      "   - view_user_audit: View audit logs for user management operations"
    );
    console.log(
      "   - view_invoice_audit: View audit logs for invoice operations"
    );
    console.log("   - view_buyer_audit: View audit logs for buyer operations");
    console.log(
      "   - view_product_audit: View audit logs for product operations"
    );

    console.log("\n🚀 Next steps:");
    console.log(
      "   1. Restart your backend server to load the new audit models"
    );
    console.log(
      "   2. Access the audit management interface at /audit-management"
    );
