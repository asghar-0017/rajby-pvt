    ...
    auditLogs.forEach((log, index) => {
      ...
      Object.keys(newValues).forEach(key => {
        ...
        if (value === null || value === undefined || value === '') {
          ...
          console.log(`   ✅ ${key}: ${typeof value === 'object' ? `[${Array.isArray(value) ? 'array' : 'object'}]` : String(value).substring(0, 50)}`);
        }
      });
      
      // Check for missing fields that should be present
      const expectedFields = [
        'invoiceRefNo', 'companyInvoiceRefNo', 'internal_invoice_no', 'transctypeId',
        'sellerFullNTN', 'sellerCity', 'buyerCity'
      ];
      
      console.log('\n🔍 Missing Expected Fields:');
      expectedFields.forEach(field => {
        if (!(field in newValues) || newValues[field] === null || newValues[field] === undefined || newValues[field] === '') {
