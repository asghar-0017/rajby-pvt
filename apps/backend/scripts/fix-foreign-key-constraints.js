  ...
  try {
    ...
    if (orphanedAuditLogs.length > 0) {
      ...
      orphanedAuditLogs.forEach(log => {
        console.log(`   - ID: ${log.id}, User ID: ${log.user_id}, Email: ${log.user_email}`);
      });
    ...
    if (orphanedBackups.length > 0) {
      console.log(`⚠️  Found ${orphanedBackups.length} orphaned backup records`);
      console.log('   Sample orphaned records:');
      orphanedBackups.forEach(backup => {
        console.log(`   - ID: ${backup.id}, User ID: ${backup.user_id}, Email: ${backup.user_email}`);
      });
      ...
      console.log(`✅ Fixed ${updateResult.affectedRows} orphaned backup records`);
    } else {
    ...
    console.log(`📊 Found ${constraints.length} audit-related foreign key constraints:`);
    ...
    console.log('   • Fixed any orphaned references by setting user_id to NULL');
    console.log('   • Verified foreign key constraints are properly configured');
    console.log('');
