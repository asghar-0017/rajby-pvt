...
async function debugAuditInvoiceItems() {
  ...
  try {
    ...
    if (newValues.invoice_items && newValues.invoice_items.length > 0) {
      ...
      console.log(
        `   product_name: ${item.product_name} (${typeof item.product_name})`
      );
      console.log(
        `   product_code: ${item.product_code} (${typeof item.product_code})`
      );
      ...
      console.log(
        `   total_price: ${item.total_price} (${typeof item.total_price})`
      );
      console.log(
        `   product_description: ${item.product_description} (${typeof item.product_description})`
      );
