...
async function testInvoiceItemFields() {
  ...
  try {
    ...
    console.log(
      `📋 Testing with invoice: ${invoice.invoice_number} (ID: ${invoice.id})`
    );
    ...
    if (items.length > 0) {
      ...
      console.log(
        `   productDescription: ${sampleItem.productDescription || "NULL"}`
      );
      ...
      console.log(
        `   product_description: ${mappedItem.product_description || "NULL"}`
      );

      // Check for N/A values
      const hasNAValues = Object.values(mappedItem).some(
      ...
      if (hasNAValues) {
        ...
        console.log(
          "   This is expected if the invoice items have missing data"
        );
      } else {
        console.log(
