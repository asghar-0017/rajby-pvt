...
async function verifyBackupSystem() {
  ...
  try {
    ...
    for (const tenant of tenants) {
      ...
      try {
        ...
        if (backupTables.length > 0) {
          ...
          const keyColumns = ['id', 'original_invoice_id', 'backup_type', 'invoice_data', 'created_at'];
        ...
        if (summaryTables.length > 0) {
          ...
          console.log(`   📊 invoice_backup_summary has ${columns.length} columns`);
        ...
        try {
          ...
          const [insertResult] = await tenantConnection.execute(`
            ...
            VALUES (?, ?, ?, ?)
          `, [testData.original_invoice_id, testData.backup_type, testData.invoice_data, testData.created_at]);
          
          console.log(`   ✅ Test backup inserted successfully (ID: ${insertResult.insertId})`);
        ...
        console.log(`   ❌ Error verifying tenant ${tenant.database_name}: ${error.message}`);
      }
    }
    ...
    console.log('');
    console.log('📋 Summary:');
    console.log(`   • ${totalBackupTables}/${tenants.length} tenant databases have invoice_backups table`);
    console.log(`   • ${totalSummaryTables}/${tenants.length} tenant databases have invoice_backup_summary table`);
    console.log('   • Backup system is ready for use');
    console.log('');
    
    if (totalBackupTables === tenants.length && totalSummaryTables === tenants.length) {
      console.log('✅ All tenant databases are properly configured for backups!');
      console.log('');
      console.log('🚀 The backup system is fully operational and will automatically:');
      ...
      console.log('   • Create backups when invoices are edited (both old and new data)');
