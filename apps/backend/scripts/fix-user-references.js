...
async function fixUserReferences() {
  ...
  try {
    ...
    if (users.length === 0) {
      ...
      console.log(`✅ User ID 5 exists: ${users[0].email} (${users[0].first_name} ${users[0].last_name})`);
    }
    ...
    if (missingUsers.length > 0) {
      console.log(`⚠️  Found ${missingUsers.length} audit logs with missing user references:`);
      missingUsers.forEach(user => {
        console.log(`   - User ID: ${user.user_id}, Email: ${user.user_email}, Name: ${user.user_name}`);
      });
    ...
    if (missingBackupUsers.length > 0) {
      console.log(`⚠️  Found ${missingBackupUsers.length} backup records with missing user references:`);
    ...
    if (verifyUsers.length > 0) {
      console.log(`✅ User ID 5 verified: ${verifyUsers[0].email} (${verifyUsers[0].first_name} ${verifyUsers[0].last_name})`);
    } else {
    ...
    console.log('   • Fixed orphaned references in audit logs and backup records');
    console.log('   • Verified user ID 5 exists');
    console.log('');
    console.log('🔧 Next steps:');
    console.log('   1. Restart your backend server');
    console.log('   2. Test creating/updating invoices');
    console.log('   3. The backup system should now work without foreign key errors');
