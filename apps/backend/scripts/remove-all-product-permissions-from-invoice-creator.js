...
async function removeAllProductPermissionsFromInvoiceCreator() {
  let connection;
  try {
      ...
      try {
        ...
        if (permission.length > 0) {
          ...
          if (result.affectedRows > 0) {
            console.log(
              `✓ Removed ${permissionName} from invoice_creator role`
            );
          } else {
            console.log(
              `- ${permissionName} was not assigned to invoice_creator role`
            );
          }
        } else {
    ...
    // Add back only the correct permissions for invoice_creator
    ...
    console.log("\nAdding correct permissions to invoice_creator role...");

    for (const permissionName of correctPermissions) {
      try {
        const [permission] = await connection.execute(
          "SELECT id FROM permissions WHERE name = ?",
          [permissionName]
        ...
        if (permission.length > 0) {
          ...
          if (existing.length === 0) {
            ...
            console.log(`✓ Added ${permissionName} to invoice_creator role`);
          } else {
            console.log(
              `- ${permissionName} already assigned to invoice_creator role`
            );
          }
        } else {
          console.log(`- Permission '${permissionName}' not found`);
    ...
    console.log("- Buyers");
    console.log(
      "- All product management buttons (CREATE, DELETE, BULK OPERATIONS)"
    );
  } catch (error) {
...
removeAllProductPermissionsFromInvoiceCreator();
