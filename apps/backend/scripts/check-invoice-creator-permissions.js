...
async function checkInvoiceCreatorPermissions() {
  ...
  try {
    ...
    if (hasProductView) {
      console.log(
        "❌ ERROR: invoice_creator should NOT have product.view permission!"
      );
      console.log(
        "This is why they can see the Products page and SELECT button."
      );
    } else {
      console.log(
        "✅ CORRECT: invoice_creator does not have product.view permission"
      );
    }
  } catch (error) {
