    ...
    for (const log of auditLogs) {
      ...
      if (log.new_values) {
        ...
        if (newValues.invoice_items) {
          console.log(
            `   ✅ New Values contains invoice_items: ${newValues.invoice_items.length} items`
          );
          if (newValues.invoice_items.length > 0) {
            console.log(
              `   📦 Sample item: ${newValues.invoice_items[0].product_name || "N/A"}`
            );
          }
        } else {
      ...
      if (log.old_values) {
        ...
        if (oldValues.invoice_items) {
          console.log(
            `   ✅ Old Values contains invoice_items: ${oldValues.invoice_items.length} items`
          );
        } else {
