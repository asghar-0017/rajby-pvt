  ...
  try {
    ...
    if (newValues.invoice_items && Array.isArray(newValues.invoice_items)) {
      ...
      newValues.invoice_items.forEach((item, index) => {
        ...
        console.log(
          `     Total Value: ${item.totalValues ? `$${parseFloat(item.totalValues).toFixed(2)}` : "N/A"}`
        );
        ...
        console.log(
          `     FED Payable: ${item.fedPayable ? `$${parseFloat(item.fedPayable).toFixed(2)}` : "N/A"}`
        );
        console.log(
          `     Advance Income Tax: ${item.advanceIncomeTax ? `$${parseFloat(item.advanceIncomeTax).toFixed(2)}` : "N/A"}`
        );
        console.log(
          `     Discount: ${item.discount ? `$${parseFloat(item.discount).toFixed(2)}` : "N/A"}`
        );
      });
    } else {
    ...
    const additionalFields = Object.entries(newValues).filter(
      ([key]) => !coveredFields.has(key)
    );
    ...
    if (additionalFields.length > 0) {
      ...
      additionalFields.forEach(([key, value]) => {
        console.log(
          `   ${key}: ${value === null ? "N/A" : typeof value === "object" ? JSON.stringify(value) : String(value)}`
        );
      });
    }
    ...
    console.log(
      "   - Additional Information (yellow section for extra fields)"
    );
  } catch (error) {
