...
async function fixAuditPermissionIndexes() {
  ...
  try {
    ...
    console.log(`📊 Current indexes on audit_permissions table: ${indexes.length}`);
    
    if (indexes.length > 64) {
      console.log(`⚠️  Table has ${indexes.length} indexes (over the 64 limit)`);
        ...
        if (indexNames.length > 1) {
          ...
          for (const indexName of indexesToRemove) {
            try {
              ...
              await connection.execute(`ALTER TABLE audit_permissions DROP INDEX \`${indexName}\``);
      ...
      const [finalIndexes] = await connection.execute('SHOW INDEX FROM audit_permissions');
